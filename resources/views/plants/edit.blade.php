<x-layout>
    ...
    <header class="text-center">
        ...
        <p class="mb-4">{{$plant->name}}</p>
    </header>
    
    <form method="POST" action="/plant/{{$plant->id}}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="mb-6">
            <label
        ...
        name="positionx"
        id="positionx"
        value="{{$plant->positionx}}" readonly />

        <input
        type="hidden"
        name="positiony"
        id="positiony"
        value="{{$plant->positiony}}" readonly />
    ...
    <script>
        ...
        context.beginPath();
        context.arc(30 + {{$plant['positionx']}}, 30 +  {{$plant['positiony']}}, 30, 0, 2 * Math.PI);
